      ...
      <div className="relative z-10 max-w-7xl w-full grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        <div className="space-y-10">
          ...
          <div className="space-y-4">
            ...
            >
              Quant Logic.<br/>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-tiktok-cyan via-tiktok-cyan/80 to-tiktok-cyan/40">Market Scale.</span>
            </motion.h1>
            <motion.p 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              Architecting <span className="text-text-main">High-Frequency Data Pipelines</span> and Risk Management Systems. Transitioning from Software Engineering to Quantitative Finance via WorldQuant University.
              <span className="block mt-4 text-tiktok-cyan text-lg italic border-l-2 border-tiktok-cyan pl-4">Capstone: MScFE + Applied Data Science Lab Integration.</span>
            </motion.p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-6 relative">
          {[
            { icon: Cpu, title: 'Quantitative', desc: 'MScFE Track', target: 'MScFE' },
            { icon: Database, title: 'Data Lab', desc: 'Pandas & NumPy', target: 'Data Science' },
            { icon: Code, title: 'Engineering', desc: 'Fullstack Architect', target: 'MERN Stack' },
            { icon: ShieldCheck, title: 'Risk Guard', desc: 'Defensive Trading', target: 'Sentinel' },
          ].map((item, i) => (
            <motion.div
              key={item.title}

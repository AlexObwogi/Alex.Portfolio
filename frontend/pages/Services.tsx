          ...
          {services.map((service, idx) => (
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <Link 
                  to={`/#client-work?filter=${encodeURIComponent(service.title)}`}
                  className="inline-flex items-center gap-4 py-5 px-10 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] text-tiktok-cyan hover:bg-tiktok-cyan hover:text-black hover:border-tiktok-cyan transition-all w-full justify-center"
                >
                  View_Client_Applications <ChevronRight className="w-4 h-4" />
                </Link>
              </div>
            </motion.div>
          ))}

...
export const downloadResumePDF = (tailoredContent: TailoredContent | null) => {
  ...
  doc.setFontSize(24);
  const name = tailoredContent?.header?.name || 'ALEX N. OBWOGI';
  doc.text(name.toUpperCase(), margin, y);
  y += 10;
  
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  const location = tailoredContent?.header?.location || 'Nairobi, Kenya';
  const email = tailoredContent?.header?.email || 'obwogialex728@gmail.com';
  const phone = tailoredContent?.header?.phone || '+254 706 050 538';
  
  doc.text(`${location} | ${email} | ${phone} | github.com/AlexObwogi`, margin, y);
  y += 15;
  ...
  const summaryText = tailoredContent?.summary || "Strategic Quantitative Software Engineer transitioning from high-performance systems architecture to mathematical finance, catalyzed by intensive research at WorldQuant University's MScFE and Applied Data Science Lab. Specialized in bridging the gap between core engineering and financial modeling through rigorous application of Stochastic Calculus, Risk Management, and Predictive Modeling. Committed to architecting low-latency, resilient financial systems using executive-level precision and cross-disciplinary expertise.";
  ...
  // Education Section
  const eduToRender = tailoredContent?.education || [
    { 
      degree: "MSc in Financial Engineering", 
      institution: "WorldQuant University", 
      period: "Jun 2026 - 2028", 
      details: "Focus on Stochastic Calculus and Risk Management. Commencing June 8, 2026." 
    },
    { 
      degree: "Applied Data Science Lab", 
      institution: "WorldQuant University", 
      period: "May 2026 - Aug 2026", 
      details: "Predictive modeling on large-scale financial datasets." 
    },
    { 
      degree: "BSc Computer Science", 
      institution: "Kirinyaga University", 
      period: "2022 - 2026", 
      details: "High-Performance Systems & Cloud Defense" 
    }
  ];
  if (y > 250) {
    doc.addPage();
